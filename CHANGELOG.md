...
首版做成了 `cards.json` 里的 `claimedTotal` 手动声明——**那正是要修的失败模式**：忘了声明就不检查，而忘了正是问题本身。已删。

改为 `validate_post.py --cards <cards.json>`：从 publish 块里正则揪出「N条」「N个坑」这类承诺，与实际条目数比对，**不需要任何声明**。实测同时抓出正文的「28条」和封面 prompt 的「28个坑」。内容已统一改为 29。

**② 话题标签内一律不脱敏**

`#雷*恩充电桩` **会让标签直接失效**——带星号的话题聚合不到、搜不到。标签是搜索键不是正文。验证器检查脱敏时先摘掉 `#...` 再扫。
...
| 不出现（产品线介绍、单推、纯避坑清单） | **雷迪恩**全称 —— 脱敏是为规避提竞品的侵权风险，自家没有这个风险 |

验证器已按此判定：检测到竞品品牌名（脱敏前后皆算）才要求本品脱敏。

⚑ **连锁反应**：本品写全称会提高复写查重风险——原文往往也写全称，改完立刻撞上「装的是雷迪恩磐石」8 字。已改句式规避，并写进 compliance。

**④ emoji 要对齐原文的功能位**

原文 emoji 不是装饰是功能位。复写换符号、不换位置和功能。

⚠️ **收藏引导那一位最容易漏。** 模板14 收藏 311 > 点赞 199，「先🐴住」是直接原因，而首版复写把它漏了——**emoji 少一个不明显，丢掉的是一个转化动作**。已在 `rewrite.md` 做成功能位对照表。
